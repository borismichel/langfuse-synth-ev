...
def _get_all_scores(base: str, name: str, limit_pages: int = 30) -> list[dict]:
    out: list[dict] = []
    page = 1
...
def run_verify(cfg: Config, state: RunState, *, log=print) -> VerifyReport:
    ...
    try:
        ...
        exists = 0
        for tid in reserved:
            r = requests.get(f"{base.rstrip('/')}/api/public/traces/{tid}", auth=_auth(), timeout=20)
    ...
    try:
        scores = _get_all_scores(base, "user_disagreement")
    ...
    try:
        aq = _get_all_scores(base, "answer_quality")
