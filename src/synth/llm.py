...
The demo-time model calls (the live playground submission and the experiment task)
are the only real model calls this kit makes — the seed path is deterministic and
model-free. This module reads the provider selection from the environment and routes
those calls through the Anthropic **or** OpenAI SDK behind one small interface.
...
class LLMClient:
    ...
    def complete(
        ...
        *,
        ...
        system prompt is passed separately, mirroring the Anthropic API shape).
        """
        client = self._client()
        if self.provider == "anthropic":
...
def get_llm(model: str | None = None) -> LLMClient:
    ...
    ``model`` is the caller's default (typically the config's ``task_model``); it is
    used only when ``LLM_MODEL`` is unset, and is ignored for a non-Anthropic provider
    whose default differs (an Anthropic model id would not exist there).
    """
