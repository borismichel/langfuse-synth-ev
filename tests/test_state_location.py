...
def test_state_never_lands_in_the_artifact_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("SYNTH_STATE_DIR", raising=False)
    assert (tmp_path / "out") not in state_dir().parents
...
def test_seed_state_round_trips_across_processes(monkeypatch, tmp_path):
    ...
    Simulated by clearing the module's view and re-reading from disk — what the
    verify container does when it starts with only the spool volume attached.
    """
    shared = tmp_path / "spool"
    monkeypatch.setenv("SYNTH_STATE_DIR", str(shared))

    st = _minimal_state()
    st.save()  # seed container writes...
...
def _minimal_state() -> RunState:
    return RunState(
        ...
        grant_effective_date="2026-01-01",
        drift_window="30d",
