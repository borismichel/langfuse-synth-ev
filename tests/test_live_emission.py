...
from synth.live.trace import TRACE_NAME, emit_live_trace, live_subsidy_eligibility
...
def test_a_live_submission_sees_todays_policy(emitter):
    """The seeded pool's stale-window text is a different story with a different mechanism;
    a submission runs against the policy that is active now, and a v1 decision that did not
    apply the grant is flagged — which is the bug the presenter opens the tool step to see."""
    cfg = load_config("config/demo.yaml")
    app, decision = _submission(cfg)
    report, ignored = live_subsidy_eligibility(cfg, app, decision)

    assert report["applicable_subsidies"][0]["amount_eur"] == cfg.golden_path.grant_amount_eur
    assert ignored is (decision.applied_grant_eur == 0)
